...
namespace MyDodos.Service.TimeOff
{
    public class TimeOffService : ITimeOffService
    {
        ...
        public TimeOffService(IConfiguration configuration, ILeaveRepository leaveRepository, ITimeOffRepository timeoffRepository, IHolidayRepository holidayrepository, IStorageConnect storage, IDocRepository docRepository, IMailRepository mailrepository)
        {
        ...
        public Response<LeaveRequestModelMsg> AddNewTimeOffRequest(TimeoffRequestModel leave) 
        {
            ...
            try
            {
                var Msg = _timeoffRepository.AddNewTimeOffRequest(leave);
                //Msg.ErrorMsg = "Save Data";
                if(Msg.RequestID > 0)
                {
                    var TimeoffLeavedetails = _timeoffRepository.GetTimeOffLeave(Msg.RequestID, leave.TenantID, leave.LocationID);
                ...
                leave.TimeoffID = Msg.RequestID;
                leave.EntityType  = "Timeoff";
                var objper = _mailrepository.GetMailReportName(leave.EmpID);
                  if(Msg.Msg == "Submitted Successfully")
                    {
                        leave.EntityName = Convert.ToString(EntityType.TimeOffLeaveApply);
                    }
                    if(Msg.Msg == "Updated Successfully")
                    {
                        if(leave.LeaveStatus  == "Approved")
                        {
                            ...
                            leave.EntityName = Convert.ToString(EntityType.TimeOffLeaveApproved);
                        }
                        else
                        {
                            var empmail = objper[0].EmployeeEmail;
                            objper[0].EmployeeEmail = objper[0].ReportingEmail;                            
                            objper[0].ReportingEmail = empmail;
                            leave.EntityName = Convert.ToString(EntityType.TimeOffRejected);
                        }
                    }
                    mailID = SendLeave(leave, objper);
                    if(Msg.Msg == "Submitted Successfully")
                    {
                        leave.EntityName = Convert.ToString(EntityType.TimeOffSumbit);
                ...
                //         {
                //             if (item.TemplateName == "TimeOffLeave Apply Template")
                //             {
                ...
                //             {
                //                 var tag = _docRepository.GetMetatag(leave.ProductID,TempName.TemplateID, leave.TimeoffID, "Timeoff");
                ...
                //                     NotifySubject = "Timeoff: Testing",
                //                     //NotifySubject = "Timeoff: " + leave.LeaveStatus,
                //                     NotifyBody = fileContent,
                //                     Entity = "Timeoff: " + leave.LeaveStatus,
                //                     NotifyID = leave.EmpID,
                ...
                //             if (leave.LeaveStatus == "Approved")
                //             {
                //                 if (item.TemplateName == "TimeOffLeave Approved Template")
                //                 {
                //                     var det = _docRepository.GetDocRepository(leave.TenantID, leave.TenantID, "Tenant", Convert.ToInt32(_configuration.GetSection("ProductID").Value), "Email Template");
                //                     var objdocpath = det.Data;
                //                     if (objdocpath.Count > 0)
                ...
                //             else
                //             {
                //                 if (item.TemplateName == "TimeOff Reject Template")
                //                 {
                //                     var det = _docRepository.GetDocRepository(leave.TenantID, leave.TenantID, "Tenant", Convert.ToInt32(_configuration.GetSection("ProductID").Value), "Email Template");
                //                     var objdocpath = det.Data;
                //                     if (objdocpath.Count > 0)
        ...
        // public Response<string> SumbitMailLeave(TimeoffRequestModel leave)
        // {
        ...
        //             foreach (var item in objtemp)
        //             {
        //                 if (item.TemplateName == "TimeOff Sumbit Template")
        //                 {
        //                     var det = _docRepository.GetDocRepository(leave.TenantID, leave.TenantID, "Tenant", Convert.ToInt32(_configuration.GetSection("ProductID").Value), "Email Template");
        //                     var objdocpath = det.Data;
        //                     if (objdocpath.Count > 0)
        ...
        //     return response;
        // }
        public int SendLeave(TimeoffRequestModel leave, List<MailNotifyBO> standarddata)
        {
            int mailID = 0;
            ...
            oParamValue.Add("requestId", Convert.ToString(leave.TimeoffID));
                    ...
                    foreach (var item in standarddata)
                    {
                        //emlContent = "Your Verification OTP is : " + otp;
                                MailModel oNBO = new MailModel
                                {
                                    ...
                                    Entity = "Timeoff: " + leave.LeaveStatus,
                                    NotifyID = leave.TimeoffID,
                                    EntityId = leave.TimeoffID,
                                    TenantID = leave.TenantID,
                                };
                               mailID = _mailrepository.SendMail(oNBO).Result;
        ...
        public void SendSumbit(TimeoffRequestModel leave, List<MailNotifyBO> standarddata)
        {
            int mailID = 0;
            string emlContent = string.Empty;
            List<TemplateBO> TagS = new List<TemplateBO>();
        ...
        public Response<GetTimeoffRequestBO> GetTimeoffRequestList(int TenantID, int YearId, int LocationID, int EmpId)
        {
            Response<GetTimeoffRequestBO> response;
            try
            {
                GetTimeoffRequestBO objleave = new GetTimeoffRequestBO();
                LeaveRequestModel leav = new LeaveRequestModel();
                List<TimeoffRequestModel> objteam = new List<TimeoffRequestModel>();
                // int result = 0;               
               ...
               objleave.OutStandLeave = _timeoffRepository.GetTimeOffLeaveStatus(leav);
               }
               if(leav.EmpID > 0)
               {
                leav.LeaveStatus = "Submitted";
               objleave.OutStandRequest = _timeoffRepository.GetTimeOffLeaveStatus(leav);
                    ...
                    foreach(var obj in robj)
                    {
                        ...
                        obj.YearID = YearId;
                        var team = _timeoffRepository.GetTimeOffLeaveStatus(obj);
                        objteam.AddRange(team); 
                    }
                    objleave.TimeOffTeamMember =  objteam;
               }
                objleave.YearDetails = _holidayrepository.GetYearDetails(YearId,TenantID,LocationID);
            ...
            //    }
               objleave.Holiday = _holidayrepository.GetHolidayList(TenantID, YearId);
               objleave.UpcomingHoliday = objleave.Holiday.Where(s => Convert.ToDateTime(s.HDate).Date > DateTime.Now.Date && s.HType == "Mandatory").FirstOrDefault();
                response = new Response<GetTimeoffRequestBO>(objleave, 200, "Data Retraived");
            }
            catch (Exception ex)
            {
                response = new Response<GetTimeoffRequestBO>(ex.Message, 500);
            }
            return response;
        }
        public Response<List<HRVwBeneftisLeave_BO>> GetTimeOffLeaveCategory(LeaveRequestModel leave)
        {
            Response<List<HRVwBeneftisLeave_BO>> response;
            try
            {
                var result = _timeoffRepository.GetTimeOffLeaveCategory(leave);               
        ...
        public Response<GetMyTimeoffList> GetMyTimeoffList(GetMyTimeoffList objresult)
        {
            Response<GetMyTimeoffList> response;
            try
            {
                var result = _timeoffRepository.GetMyTimeoffList(objresult);         
                if (result.objMyTimeoffList.Count() == 0)
                {      
                    response = new Response<GetMyTimeoffList>(result, 200, "Data Not Found");
                }
                else
                {      
                    response = new Response<GetMyTimeoffList>(result, 200, "Data Retraived");
                }
            }
            catch (Exception ex)
            {
                response = new Response<GetMyTimeoffList>(ex.Message, 500);
            }
            return response;
        }
        public Response<GetTimeoffRequestBO> GetManagerRequestList(LeaveRequestModel _objleave)
        {
            Response<GetTimeoffRequestBO> response;
            try
            {
